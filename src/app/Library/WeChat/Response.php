...
//        '填写图片的mediaId','填写图片的mediaId');
    ...
    {
        $token = getWeChatConfigByChannelId($this->config['channelId']);
     ...
     * @return bool
     */
    public function responseMessage($rawContent)
    {
        if(empty($rawContent)){
            return false;
        }
        ...
        // 禁止xml实体解析，防止xml注入
        $requestXml = simplexml_load_string($rawContent, 'SimpleXMLElement', LIBXML_NOCDATA);
//        var_dump($requestXml,$requestXml->MsgType);
//        return;
 ...
 //                   $this->_doClick($requestXml);
                }elseif ($event=='VIEW') {// 连接跳转事件
 //                   $this->_doView($requestXml);
                }elseif ($event=='SCAN') { // 二维码扫码事件
                    // var_dump($requestXml);
                    $response = $this->_doSCAN($requestXml);
                }else {

                }
                break;
            case 'text': // 文本消息
                $response = $this->doText($requestXml);
                break;
            case 'image'://图片消息
                $this->doImage($requestXml);
                break;
            case 'voice'://语音消息
//                $this->_doVoice($requestXml);
                break;
            case 'video'://视频消息
//                $this->_doVideo($requestXml);
                break;
            case 'shortvideo'://短视频消息
//                $this->_doShortvideo($requestXml);
                break;
            case 'location'://位置消息
//                $this->_doLocation($requestXml);
                break;
            case 'link'://链接消息
//                $this->_doLink($requestXml);
                break;
        }
        return $response;
    }
   ...
   /* public function menuDelete(){
    ...
    public function menuSet($menu) {
        ...
        $data = $menu;
        $resultObject = json_decode($this->_request('post',$url, $data));
        if ($resultObject->errcode == 0) {
            return true;
        } else {
            echo $resultObject->errmsg, '<br>';
            return false;
        }
    }*/
	...
	/*
    //点击菜单
	private function _doClick($requestXml) {
        $content = '你点击了菜单，携带的KEY为: ' . $requestXml->EventKey;
        $this->_msgText($requestXml->FromUserName, $requestXml->ToUserName, $content);
    }
	...
	*/
    ...
    /**
     * @param $requestXml 事件内容
     * @auth chenqianhao 68527761@qq.com
     */
    private function _doSCAN($requestXml) {
        return $requestXml;
        ...
        // $content = '二维码扫码关注。';
        // $response = $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
        // return $response;
    ...
    /******************用户输入消息********************/
    // 文本消息
    private function doText($requestXml)
    {
        //接受文本信息
        ...
        if ($content == '学习') {
            ...
            $response = $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $response_content);
        } elseif ($content == '帮助') {
            ...
            $response = $this->sendMessageImage($requestXml->FromUserName, $requestXml->ToUserName, $imgMediaId, true);
        }
        /*elseif ('音乐' == $content) {
            $music_url = '音乐地址';
            $ha_music_url = '音乐地址';
            $thumbMediaId = '缩略图mediaId';
            $title = '等你爱我';
            $desc = '等你爱我-等到地老天荒';
            $this->_msgMusic($requestXml->FromUserName, $requestXml->ToUserName, $music_url, $hq_music_url, $thumbMediaId, $title, $desc);
        } elseif ('新闻' == $content) {
            $item_list = array(
                array('title' => '生日快乐', 'desc' => '其实你应该用母亲的方式回报母亲', 'picurl' => '图片地址', 'url' => 'http://www.baidu.com'),
                array('title' => '生日快乐', 'desc' => '其实你应该用母亲的方式回报母亲', 'picurl' => '图片地址', 'url' => 'http://www.baidu.com'),
            );
            $this->_msgNews($requestXml->FromUserName, $requestXml->ToUserName, $item_list);
        } else {
            //图灵机器人接入
            $url = 'http://www.tuling123.com/openapi/api?key=' . $this->_appkey . '&info=' . $content . '&userid=' . $requestXml->FromUserName;
            // $data['key'] = $this->_appkey;//
            // $data['info'] = $content;//用户输入的内容
            // $data['userid'] = $requestXml->FromUserName;
            ...
            $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $response_content->text);
        }*/
        return $response;
    }

    // 图片消息
    private function doImage($requestXml)
    {
        $content = '上传图片成功，请输入 图片 随机获取一张图片' . $requestXml->MediaId;
        $this->_img_list[] = $requestXml->MediaId;
        $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
    }
//	//视频消息
//	private function _doVideo($requestXml){
//    $content = '上传视频成功';
//    $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
//}
//	//语音消息
//	private function _doVoice($requestXml){
//    $content = '你TM没吃饭吗？，大声点';
//    $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
//}
//	//短视频
//	private function _doShortvideo($requestXml){
//    $content = '短视频发送成功';
//    $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
//}
//	//位置信息
//	private function _doLocation($requestXml){
//    // $content = '你的坐标为,经度:'.$requestXml->Location_Y.',纬度:'.$requestXml->Location_X . "\n" . '你所在的位置为：' . $requestXml->Label;
//    // $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
//    //利用位置获取信息
...
//    $page_num = 0;//第几页
//    $scope = 1;//1基本信息，2详细信息
//    $location = $requestXml->Location_X.','.$requestXml->Location_Y;
//    $radius = 2000;//方圆两公里
...
//    $resultObject = json_decode($result);
//    $re_list = array();
//    foreach($resultObject->results as $re){
//        $r['name'] = $re->name;
...
//    //implode变为字符串
//    $re_str = implode("\n",$re_list);
//    $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $re_str);
//
//}
//	//链接消息
//	private function _doLink($requestXml){
//    $content = '你要给宝宝看撒？';
//    //$this->_img_list[] = $requestXml->MediaId;
//    $this->sendMessageText($requestXml->FromUserName, $requestXml->ToUserName, $content);
//}
     ...
     * @return string
     */
	private function sendMessageText($toUserName, $fromUserName, $content)
    {
        return sprintf($this->messageTemplate['text'], $toUserName, $fromUserName, time(), $content);
    }
     ...
     * @return string
     */
    private function sendMessageImage($toUserName, $fromUserName, $file, $isId = false)
    {
        // 判断是不是mediaId
        if ($isId) {
            $mediaId = $file;
...
//            $resultObject = $this->uploadTmp($file, 'image');
//            $mediaId = $resultObject->mediaId;
        }
        // 拼凑xml图片发给微信平台，然后平台返回图片给用户
        return sprintf($this->messageTemplate['image'], $toUserName, $fromUserName, time(), $mediaId);
    }
//	//发送音乐
//	private function _msgMusic($toUserName, $fromUserName, $music_url, $hq_music_url, $thumbMediaId, $title='', $desc='') {
//    $response = sprintf($this->messageTemplate['music'], $toUserName, $fromUserName, time(), $title, $desc, $music_url, $hq_music_url, $thumbMediaId);
//    die($response);
//}
//	//发送新闻
//	private function _msgNews($toUserName,$fromUserName,$item_list=array()){
//    //拼凑文章部分
//    $item_str = '';
//    foreach ($item_list as $item) {
//        $item_str .= sprintf($this->messageTemplate['news_item'],$item['title'],$item['desc'],$item['picurl'],$item['url']);
//    }
//    //拼凑主体部分
//    $response = sprintf($this->messageTemplate['news'], $toUserName, $fromUserName, time(), count($item_list), $item_str);
//    die($response);
//}
//	/**
//     * 上传临时素材：图片,语音，视频，缩略图
//     * 储存到微信公众平台服务器，3天
//     * 可通过上传后返回的mediaId再次去取得该图片
//     */
...
//    $resultObject = json_decode($result);
//    //写入文件
//    file_put_contents($token_file, $resultObject->access_token);

...
/**
 * Model工厂模式
 * 替换以下文件：vendor/tmtbe/swooledistributed/src/Server/CoreBase/ModelFactory.php
 * Created by PhpStorm.
 * User: zhangjincheng
 * Date: 16-7-15
 * Time: 下午12:03
...
class ModelFactory
{
    ...
    public function getModel($old_model)
    {
        ...
        if (class_exists($class_name)) {
            ...
            $class_name2 = "app\\Server\\$model";
            if (class_exists($class_name)) {
                $model_instance = new $class_name;
                $model_instance->core_name = $old_model;
                $this->addNewCount($old_model);
            } elseif (class_exists($class_name2)) {
                $model_instance = new $class_name2;
                $model_instance->core_name = $old_model;
                $this->addNewCount($old_model);
            } else {
